...
Page({
  ...
  // 提交信息
  save: function () {
    let hairCareDays = this.data.hairCareDays
    ...
    let ispastover = this.judgetimeRange(endTime, currentTime, hairCareDays)
    // 如果超过今天
    if (isover) {
      wx.showToast({
        icon: 'none',
    ...
    let params = {
      hairCareDays: hairCareDays,
      hairCarePrevTime: endTime,
      uid: uid
    }
    setHairCare(params).then(res => {
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getHairCareRcds(params).then(res => {
