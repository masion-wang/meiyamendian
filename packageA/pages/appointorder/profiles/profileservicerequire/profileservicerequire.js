...
Page({
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getServiceRequireRcds(params).then(res => {
...
          
  ...
  bindTextAreaBlur: function (e) {
    ...
    this.setData({
      serviceRequire: e.detail.value
    })
  },
  // 保存
  save: function () {
    let uid = this.data.uid
    let serviceRequire = this.data.serviceRequire
    let params = {
      uid: uid,
      serviceRequire: serviceRequire
    }
    setServiceRequest(params).then(res => {
      if (res.data.code == 200) {
