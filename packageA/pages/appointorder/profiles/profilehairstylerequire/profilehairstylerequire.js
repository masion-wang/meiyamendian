...
Page({
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getHairRequireRcds(params).then(res => {
        ...
        if (res.data.code == 200) {
          ...
          if (list.length > 0) {
            that.setData({
              cstmCardWish: list[0].cstmCardWish
            })
          }
        } else {
          wx.showToast({
...
          
  ...
  bindTextAreaBlur: function (e) {
    ...
    this.setData({
      hairRequire: e.detail.value
    })
  },
  // 保存
  save: function () {
    let uid = this.data.uid
    let hairRequire = this.data.hairRequire
    let params = {
      uid: uid,
      hairRequire: hairRequire
    }
    setHairRequest(params).then(res => {
      if (res.data.code == 200) {
