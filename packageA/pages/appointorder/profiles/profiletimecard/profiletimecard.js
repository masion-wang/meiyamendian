...
Page({
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getNumCardWishRcds(params).then(res => {
...
          
  ...
  bindTextAreaBlur: function (e) {
    ...
    this.setData({
      numCardWish: e.detail.value
    })
  },
  // 保存
  save: function () {
    let uid = this.data.uid
    let numCardWish = this.data.numCardWish
    let params = {
      uid: uid,
      numCardWish: numCardWish
    }
    setNumCardWish(params).then(res => {
      if (res.data.code == 200) {
