import {
  getElseContentRcds, // 获取其它历史信息
  setElseContent // 上传其它信息
...
Page({
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getElseContentRcds(params).then(res => {
  ...
  bindTextAreaBlur: function (e) {
    ...
    this.setData({
      elseContent: e.detail.value
    })
  },
  // 保存
  save: function () {
    let uid = this.data.uid
    let elseContent = this.data.elseContent
    let params = {
      uid: uid,
      elseContent: elseContent
    }
    setElseContent(params).then(res => {
      if (res.data.code == 200) {
