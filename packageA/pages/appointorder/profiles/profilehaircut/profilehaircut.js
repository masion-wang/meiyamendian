...
Page({
  ...
  // 提交信息
  save: function () {
    let hairCutDays = this.data.hairCutDays
    ...
    let ispastover = this.judgetimeRange(endTime, currentTime, hairCutDays)
    // 如果超过今天
    if (isover) {
      wx.showToast({
        icon: 'none',
    ...
    let params = {
      ...
      hairCutPrevTime: endTime,
      uid: uid
    }
    setHairCut(params).then(res => {
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      getHairCutRcds(params).then(res => {
