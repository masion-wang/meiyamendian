...
Page({
  ...
  // 提交信息
  save: function () {
    let scalpDays = this.data.scalpDays
    ...
    let ispastover = this.judgetimeRange(endTime, currentTime, scalpDays)
    // 如果超过今天
    if (isover) {
      wx.showToast({
        icon: 'none',
    ...
    let params = {
      scalpDays: scalpDays,
      scalpPrevTime: endTime,
      uid: uid
    }
    setScalp(params).then(res => {
