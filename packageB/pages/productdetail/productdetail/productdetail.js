...
Page({
  ...
  onLoad: function (options) {
    ...
    getUserProjectOrderDetail(params).then(res => {
      console.log('一般订单', res)
      if (res.data.code == 200) {
        console.log('一般订单详情', res)
        that.setData({
          userProjectOrderDto: res.data.data.userProjectOrderDto
        })
      } else {
        wx.showToast({
          icon: 'none',
