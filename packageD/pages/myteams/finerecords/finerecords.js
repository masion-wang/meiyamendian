import {
  ...
  selfFirmPunishmentRecord, // 罚款记录分页
  roomManagerCheckPunishmentRecord, // 门店驳回通过
  smallAreaManagerCheckPunishmentRecord, // 小区驳回通过
  bigAreaManagerCheckPunishmentRecord // 大区驳回通过
...
Page({
  ...
  loadMore() {
    ...
    if (!isend) {
      console.log("加载更多")
      selfFirmPunishmentRecord(params).then(res => {
        that.setData({
          back: false
        })
  ...
  finebutton: function (e) {
    ...
    if (roomcheckstate == 0) {
      ...
      roomManagerCheckPunishmentRecord(params).then(res => {
        if (res.data.code == 200) {
    ...
    if (roomcheckstate == 1 && smallareacheckstate == 0) {
      ...
      smallAreaManagerCheckPunishmentRecord(params).then(res => {
        if (res.data.code == 200) {
    ...
    if (roomcheckstate == 1 && smallareacheckstate == 1 && bigareacheckstate == 0) {
      ...
      bigAreaManagerCheckPunishmentRecord(params).then(res => {
        if (res.data.code == 200) {
